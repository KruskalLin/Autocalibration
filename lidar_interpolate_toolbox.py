...
def atgv(reconstructed_lidar_image, lidar_image, gray,
    ...
    for i in tqdm(range(step), desc='tgv step'):
        ...
        qw_x = np.append(np.zeros_like(q[:, -1, 0].reshape((q.shape[0], 1))), q[:, :-1, 0].reshape((q.shape[0], q.shape[1] - 1)), axis=1)
        qw_w = np.append(np.zeros_like(q[:, -1, 3].reshape((q.shape[0], 1))), q[:, :-1, 3].reshape((q.shape[0], q.shape[1] - 1)), axis=1)
        qn_y = np.append(np.zeros_like(q[-1, :, 1].reshape((1, q.shape[1]))), q[:-1, :, 1].reshape((q.shape[0] - 1, q.shape[1])), axis=0)
        qn_z = np.append(np.zeros_like(q[-1, :, 2].reshape((1, q.shape[1]))), q[:-1, :, 2].reshape((q.shape[0] - 1, q.shape[1])), axis=0)

        div_q1 = (np.append(q[:, :-1, 0].reshape((q.shape[0], q.shape[1] - 1)), np.zeros_like(q[:, -1, 0].reshape((q.shape[0], 1))), axis=1) - qw_x) + (
                    np.append(q[:-1, :, 2].reshape((q.shape[0] - 1, q.shape[1])), np.zeros_like(q[-1, :, 2].reshape((1, q.shape[1]))), axis=0) - qn_z)
        div_q2 = (np.append(q[:, :-1, 3].reshape((q.shape[0], q.shape[1] - 1)), np.zeros_like(q[:, -1, 3].reshape((q.shape[0], 1))), axis=1) - qw_w) + (
                    np.append(q[:-1, :, 1].reshape((q.shape[0] - 1, q.shape[1])), np.zeros_like(q[-1, :, 1].reshape((1, q.shape[1]))), axis=0) - qn_y)
